...
async fn server_starts_and_accepts_connections() {
    ...
    assert!(res.is_ok(), "Client failed to connect to server: {:?}", res.err());
}
...
#[tokio::test]
async fn protocol_handler_list_tools() {
...
async fn protocol_handler_list_resources_with_cursor() {
    ...
    let params = PaginatedRequestParams { cursor: Some("page2".to_string()), ..Default::default() };
    let result2 = group.list_resources_with_params(&url.parse().unwrap(), params).await.unwrap();
...
async fn protocol_handler_unknown_method() {
    ...
    let unknown_msg = serde_json::json!({"method": "unknown/method", "params": {}});
    write_half.write_all(serde_json::to_string(&unknown_msg).unwrap().as_bytes()).await.unwrap();
    write_half.write_all(b"\n").await.unwrap();
    let mut response = String::new();
    let _ = reader.read_line(&mut response).await.unwrap();
...
async fn protocol_handler_list_tools_empty() {
    ...
    let url = format!("tcp://{}", addr);
    group.connect_to_server(url.parse().unwrap()).await.unwrap();
    let result = group.list_tools(&url.parse().unwrap(), Default::default()).await;
